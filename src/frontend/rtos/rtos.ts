...
export class RTOSTracker
    ...
    constructor(private context: vscode.ExtensionContext) {
        ...
        vscode.commands.executeCommand('setContext', 'cortex-gdb:showRTOS', this.enabled);
        context.subscriptions.push(
            vscode.debug.onDidStartDebugSession(this.debugSessionStarted.bind(this)),
            vscode.debug.onDidTerminateDebugSession(
                this.debugSessionTerminated.bind(this)
            ),
            vscode.debug.registerDebugAdapterTrackerFactory('cortex-gdb', this),
            ...
            vscode.commands.registerCommand('cortex-gdb.rtos.toggleRTOSPanel', this.toggleRTOSPanel.bind(this)),
            vscode.commands.registerCommand('cortex-gdb.rtos.refresh', this.update.bind(this))
        );
    }

    private settingsChanged(e: vscode.ConfigurationChangeEvent) {
        if (e.affectsConfiguration('cortex-gdb.showRTOS')) {
            const config = vscode.workspace.getConfiguration('cortex-gdb', null);
            this.enabled = config.get('showRTOS', false);
            vscode.commands.executeCommand('setContext', 'cortex-gdb:showRTOS', this.enabled);
            this.update();
...
class RTOSViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'cortex-gdb.rtos';
    ...
    private getHtmlForWebview(): string {
        ...
        if (!this.parent.enabled) {
            return /*html*/`
                ...
                <body>
                    <p>Currently disabled. Enable setting "cortex-gdb.showRTOS" or use Command "Cortex GDB: Toggle RTOS Panel" to see any RTOS info</p>
                </body>
                </html>`;
        }
