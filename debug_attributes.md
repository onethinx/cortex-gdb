...
| showServerOutput | string | Attach | Prints server output responses to the console. 'never': no server responses, 'always': all server responses, regardless of debugging state, 'smart': switches from server to GDB when debugging is active |
